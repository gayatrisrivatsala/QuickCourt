...
export default function BookingPage() {
  ...
  useEffect(() => {
    const fetchFacility = async () => {
      try {
        ...
        try {
          const blockedSlotsResponse = await facilitiesAPI.getBlockedTimeSlots(
            params.id
          );
  ...
  useEffect(() => {
    const fetchBookedSlots = async () => {
      ...
      try {
        const response = await bookingsAPI.getFacilityAvailability(
          params.id,
          bookingData.date
        );
  ...
  const isTimeSlotAvailable = (time) => {
    ...
    const selectedEnd = new Date(
      selectedStart.getTime() + bookingData.duration * 60 * 60 * 1000
    );
    ...
    if (facility?.blockedSlots && facility.blockedSlots.length > 0) {
      const hasBlockedConflict = facility.blockedSlots.some((blockedSlot) => {
        ...
        if (
          !selectedCourt ||
          blockedCourtId.toString() !== selectedCourt._id.toString()
        ) {
          return false;
        }
  ...
  const handleBooking = async () => {
    try {
      ...
      let validPlayers = bookingData.players.filter(
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="flex items-center text-gray-600 mb-2">
                  ...
                  <span>
                    {facility.address?.city || "Unknown City"},{" "}
                    {facility.address?.state || "Unknown State"}
                  </span>
                </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {facility?.sports?.map((sportData, index) => (
                      <button
                        key={index}
                        onClick={() =>
                          handleInputChange("sport", sportData.sport._id)
                        }
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600">
                          {sportData.courts?.length || 0} courts available
                        </p>
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 cursor-pointer placeholder-gray-700 text-gray-900"
                      placeholder="Select date"
                    />
                    <Calendar className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-600" />
                  </div>
                      ...
                      <div className="grid grid-cols-7 gap-1">
                        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                          (day) => (
                            <div
                              key={day}
                              className="text-center text-sm text-gray-800 py-2 font-medium"
                            >
                              {day}
                            </div>
                          )
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (dayInfo.isAvailable) {
                                handleInputChange(
                                  "date",
                                  formatDateToLocalString(dayInfo.date)
                                );
                                setShowDatePicker(false);
                              }
                            }}
                    ...
                    <input
                      ...
                      value={
                        bookingData.startTime
                          ? timeSlots.find(
                              (slot) => slot.value === bookingData.startTime
                            )?.display || bookingData.startTime
                          : ""
                      }
                      readOnly
                      onClick={() => setShowTimePicker(!showTimePicker)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 cursor-pointer placeholder-gray-700 text-gray-900 disabled:bg-gray-100 disabled:text-gray-600"
                      placeholder="Select time"
                      disabled={!bookingData.date}
                    />
                    <Clock className="absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-600" />
                  </div>
                    ...
                    <div className="absolute z-10 mt-2 bg-white border border-gray-300 rounded-lg shadow-lg p-4 w-64">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          onClick={async () => {
                            try {
                              const response =
                                await bookingsAPI.getFacilityAvailability(
                                  params.id,
                                  bookingData.date
                                );
                              setBookedSlots(response.bookings || []);
                      ...
                      {(bookedSlots.length > 0 ||
                        (facility?.blockedSlots &&
                          facility.blockedSlots.length > 0)) && (
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {facility.blockedSlots
                                    .filter((blockedSlot) => {
                                      ...
                                      const blockedCourtId =
                                        blockedSlot.court._id ||
                                        blockedSlot.court;
                                      const selectedCourt = getSelectedCourt();
                                      if (
                                        !selectedCourt ||
                                        blockedCourtId.toString() !==
                                          selectedCourt._id.toString()
                                      ) {
                                        return false;
                                      }
                  ...
                  <div className="flex items-center justify-center space-x-4">
                    <button
                      onClick={() =>
                        handleInputChange(
                          "duration",
                          Math.max(1, bookingData.duration - 1)
                        )
                      }
                      className="w-10 h-10 border border-gray-80000 rounded-full flex items-center justify-center hover:bg-gray-50 transition-colors text-gray-800 font-bold text-lg"
                    >
                      -
                    </button>
                    <span className="text-lg font-medium min-w-[4rem] text-center text-black">
                    ...
                    <button
                      onClick={() =>
                        handleInputChange(
                          "duration",
                          Math.min(4, bookingData.duration + 1)
                        )
                      }
                      className="w-10 h-10 border border-gray-800 rounded-full flex items-center justify-center hover:bg-gray-50 transition-colors text-gray-800 font-bold text-lg"
                    >
                      +
                    </button>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      ...
                      className="text-sm text-green-600 hover:text-green-700 disabled:text-gray-600 disabled:cursor-not-allowed"
                    >
                      + Add Player
                    </button>
                  </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-green-500 focus:border-green-500 placeholder-gray-700 text-gray-900"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-2 border rounded-md text-sm focus:ring-green-500 focus:border-green-500 placeholder-gray-700 text-gray-900 ${
                                player.email &&
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500 placeholder-gray-700 text-gray-900"
                ...
                <div className="pt-6">
                  <button
                    ...
                    disabled={
                      ...
                      !bookingData.date ||
                      !bookingData.startTime ||
                      !bookingData.players[0]?.name?.trim() ||
                      bookingLoading
                    }
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {getSelectedSportData()?.sport.name}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      ₹{getSelectedCourt()?.hourlyRate}/hour
                    </span>
                  </div>
